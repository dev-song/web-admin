...
function showProductData(path, parent) {
  ...
  readJson(path, data => {
    ...
    data.forEach(item => {
      ...
      if (!clusters.includes(clusterNumber)) {
        clusterNodes[clusterNumber] = document.createElement('div');
        clusterNodes[clusterNumber].classList.add(`data-box__cluster-${clusterNumber}`);
        itemsContainer.appendChild(clusterNodes[clusterNumber]);

        clusters.push(clusterNumber);
      }
      clusterNodes[clusterNumber].appendChild(itemContainer);

      itemsContainer.prepend(clusterNodes[clusterNumber]);
    });
