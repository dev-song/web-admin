module.exports = {
    ...
    let clusterDiv = clusters.reduce((acc, cur) => {
      ...
      obj[cur] = `
        <article class='clusters' id='cluster${cur}'>
          ...
          <button class='cluster__button-show-hide'>▼ 접기/펼치기</button>
  ...
  products(productsData) {   // productsData should be in a form of JSON
    ...
    const uploadHtml = `<a class='data__upload-link' href='/data-manager'>물건 추가</a>`;
    sectionHtml = titleHtml + uploadHtml + sectionHtml;
    ...
    for (let i = 0, len = data.length; i < len; i++) {
      ...
      const item = `
        <article class='item__container' data-id=${data[i].id}>
          ...
          <span class='item__modify-button'>수정</span>
          <a class='item__delete-link' href='/data-manager/delete/${data[i].id}'>삭제</a>
        </article>
      `;
